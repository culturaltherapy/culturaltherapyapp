    ...
    <div className="min-h-dvh bg-parchment text-ink flex flex-col">
      <header className="border-b border-line">
        ...
        <div className="h-1 w-full bg-line">
          <div className="h-1 bg-terracotta transition-all" style={{ width: `${(step / TOTAL) * 100}%` }} />
        </div>
      </header>
      ...
      <main className="flex-1 px-4 sm:px-6 py-8 sm:py-12">
        <div className="mx-auto max-w-2xl">
          ...
          {step === 11 && <StepDone s={s} onFinish={finish} saving={saving} />}
        </div>
      </main>

      <footer className="border-t border-line bg-parchment/95 sticky bottom-0" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        <div className="mx-auto max-w-2xl px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          <button onClick={prev} disabled={step === 1} className="text-sm text-ink2 disabled:text-ink3 inline-flex items-center gap-1">
...
function StepWelcome() {
  return (
    <div>
      ...
      <StepHeader kicker="Welcome to B.L.E.S.S" title="Take a breath. We'll go step by step."
        body="Onboarding takes about six minutes. Everything you share has a privacy setting — you choose what others see." />
      <ul className="mt-6 space-y-2.5 text-ink2 text-[15px]">
        <li className="flex items-start gap-2"><Icon name="check" size={18} className="text-forest mt-0.5" /> Your real name stays private. We use an alias.</li>
        <li className="flex items-start gap-2"><Icon name="check" size={18} className="text-forest mt-0.5" /> ID verification is required before you can post publicly.</li>
        <li className="flex items-start gap-2"><Icon name="check" size={18} className="text-forest mt-0.5" /> Crisis support is on every screen.</li>
      </ul>
    </div>
  );
}
...
function StepIdentity({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  return (
    <div>
      <StepHeader kicker="Step 2 · Identity" title="What should we call you?"
      ...
      <div className="space-y-5">
        <Field label="Alias" required hint="2–24 characters">
          <Input value={s.alias} onChange={(e) => patch({ alias: e.target.value })}
            placeholder="e.g. Yaa O." autoComplete="nickname" maxLength={24} />
        </Field>
        <Field label="Pronouns (optional)">
          <Input value={s.pronouns} onChange={(e) => patch({ pronouns: e.target.value })}
    ...
    <div>
      ...
      <Field label="Heritage / descent" required>
        <div className="flex flex-wrap gap-2">
          {descents.map((d) => (
            <Chip key={d} as="button" active={s.descent.includes(d)} onClick={() => patch({ descent: toggle(s.descent, d) })}>{d}</Chip>
          ))}
        </div>
      </Field>
      <div className="h-5" />
      <Field label="Languages you carry">
        <div className="flex flex-wrap gap-2">
          {langs.map((l) => (
            <Chip key={l} as="button" active={s.languages.includes(l)} onClick={() => patch({ languages: toggle(s.languages, l) })}>{l}</Chip>
          ))}
        </div>
      </Field>
    </div>
...
function StepLocation({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  return (
    <div>
      <StepHeader kicker="Step 4 · Where you are now" title="Country, and a city if you want to share."
      ...
      <label className="mt-3 inline-flex items-center gap-2 text-sm text-ink2">
        <input type="checkbox" checked={s.shareCity} onChange={(e) => patch({ shareCity: e.target.checked })} className="accent-terracotta" />
...
function StepExperience({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  function toggle(v: string) {
    patch({ experienceTags: s.experienceTags.includes(v) ? s.experienceTags.filter((x) => x !== v) : [...s.experienceTags, v] });
  }
  return (
    <div>
      <div className="text-terracotta mb-3"><Sankofa size={48} /></div>
      <StepHeader kicker="Step 5 · What you've lived through" title="Choose what you'd like a Tribe to recognise."
...
function StepDiagnosis({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  return (
    <div>
      <StepHeader kicker="Step 6 · Self-description" title="If you have a diagnosis, you can name it here. (Optional.)"
        body="We never display this by default. It exists only if you want a Tribe to know." />
      <Field label="Diagnosis (optional)">
        <Input value={s.diagnosis} onChange={(e) => patch({ diagnosis: e.target.value })} placeholder="e.g. Bipolar II, c-PTSD" />
      </Field>
      <label className="mt-3 inline-flex items-center gap-2 text-sm text-ink2">
        <input type="checkbox" checked={s.diagnosisVisible} onChange={(e) => patch({ diagnosisVisible: e.target.checked })} className="accent-terracotta" />
...
function StepPrompts({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  ...
  return (
    <div>
      ...
      <StepHeader kicker="Step 7 · Prompts" title="Answer at least two."
      ...
      <p className="mt-3 text-xs text-ink3">{s.prompts.filter((p) => p.answer.trim()).length} of 5 answered · 2 required</p>
    </div>
  );
}
...
function StepAvatar({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  ...
  return (
    <div>
      <StepHeader kicker="Step 8 · A face for the room" title="A photo helps people land."
        body="A real photo is encouraged — but a portrait, illustration, or symbol is fine too." />
      <div className="flex items-center gap-5">
        <div className="h-24 w-24 rounded-full overflow-hidden border border-line flex items-center justify-center"
          style={{ background: s.avatar ? undefined : "var(--ct-rust)", color: "var(--ct-bone)" }}>
          {s.avatar ? <img src={s.avatar} alt="Your avatar" className="h-full w-full object-cover" /> :
...
function StepID({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  ...
  return (
    <div>
      ...
      <StepHeader kicker="Step 9 · ID verification" title="One quick check. Then you're in."
        body="We verify ID through a third-party (Persona). They store the document — we only see a 'verified' flag." />
      {s.idStatus === "idle" && <Button onClick={start} size="lg">Start verification</Button>}
      {s.idStatus === "verifying" && (
        <div className="surface p-5 inline-flex items-center gap-3">
          <span className="h-3 w-3 rounded-full bg-ochre animate-pulse" /> Verifying… don't close this tab.
        </div>
      )}
      {s.idStatus === "verified" && (
...
function StepCoC({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  ...
  return (
    <div>
      ...
      <StepHeader kicker="Step 10 · Code of conduct" title="Read each clause. Then accept."
        ...
        {codeOfConduct.map((c, i) => {
          ...
          return (
            <li key={c.id} className="surface overflow-hidden">
              <button onClick={() => patch({ cocVisited: { ...s.cocVisited, [c.id]: true } })}
      ...
      <label className={`mt-5 flex items-start gap-3 text-[15px] ${allVisited ? "text-ink" : "text-ink3"}`}>
        <input type="checkbox" disabled={!allVisited} checked={s.cocAccepted}
          onChange={(e) => patch({ cocAccepted: e.target.checked })} className="mt-1 accent-terracotta" />
        <span>I've read each clause and I accept the Code of Conduct.
...
function StepDone({ s, onFinish, saving }: { s: State; onFinish: () => void; saving: boolean }) {
  return (
    <div>
      <div className="text-terracotta mb-3"><Pyramid size={56} /></div>
      <StepHeader kicker="Step 11 · You're in" title={`Welcome, ${s.alias || "friend"}.`}
