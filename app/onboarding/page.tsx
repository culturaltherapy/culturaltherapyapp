...
type State = {
  ...
  city: string;
  shareCity: boolean;
  // 5 Lived experience tags
  experienceTags: string[];
  // 6 Self-description (optional diagnosis)
  diagnosis: string;
...
export default function Onboarding() {
  ...
  // Per-step gating — mandatory enforcement
  const canContinue = React.useMemo(() => {
    switch (step) {
      ...
      case 6: return true; // diagnosis optional
      case 7: return s.prompts.filter((p) => p.answer.trim().length > 0).length >= 2;
      case 8: return true; // avatar optional, has fallback
    ...
    <div className="min-h-dvh bg-parchment text-ink flex flex-col">
      ...
      <header className="border-b border-line">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <Link href="/signin" className="text-sm text-ink3 hover:text-ink">
            Sign in
          </Link>
        </div>
      ...
      <main className="flex-1 px-4 sm:px-6 py-8 sm:py-12">
        <div className="mx-auto max-w-2xl">
          ...
          {step === 11 && <StepDone s={s} onFinish={() => router.push("/home")} />}
        </div>
      </main>

      {/* Footer nav */}
      <footer className="border-t border-line bg-parchment/95 sticky bottom-0">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          {step < TOTAL ? (
            ...
            <Button onClick={() => router.push("/home")} size="md">
              Enter the network <Icon name="arrow" size={14} />
            </Button>
          )}
        </div>
      </footer>
  ...
  return (
    <div>
      ...
      <StepHeader
        ...
        body="Onboarding takes about six minutes. Everything you share has a privacy setting — you choose what others see. You can always come back and edit."
      />
...
function StepRoots({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  ...
  return (
    <div>
      ...
      <StepHeader kicker="Step 3 · Roots" title="Where are your people from?" body="Pick all that feel like you. You can add more later." />
  ...
  return (
    <div>
      ...
      <StepHeader
        ...
        body="Pick at least one. These are not diagnoses — they're touchpoints. They're shown to your Tribe by default; you can hide them per tag later."
      />
      <div className="flex flex-wrap gap-2">
        {experienceTagPool.map((t) => (
          <Chip key={t} as="button" active={s.experienceTags.includes(t)} onClick={() => toggle(t)}>
            {t}
          </Chip>
        ))}
      </div>
  ...
  return (
    <div>
      <StepHeader
        ...
        body="We never display this by default. We never sort or rank people by it. It exists only if you want a Tribe to know."
      />
...
function StepPrompts({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  ...
  function get(q: string) {
    return s.prompts.find((p) => p.question === q)?.answer ?? "";
  }
  ...
  return (
    <div>
      <StepHeader
        ...
        body="A real photo is encouraged — but a portrait, illustration, or symbol is fine too. You can change it any time."
      />
      <div className="flex items-center gap-5">
        <div
          className="h-24 w-24 rounded-full overflow-hidden border border-line bg-bone flex items-center justify-center text-3xl text-ink3"
...
function StepID({ s, patch }: { s: State; patch: (p: Partial<State>) => void }) {
  function start() {
    ...
    // Demo: simulate Persona / Stripe Identity flow
  ...
  return (
    <div>
      ...
      <StepHeader
        ...
        body="We verify ID through a third-party (Persona). They store the document — we don't. We only see a 'verified' flag. This is required before posting publicly, and it keeps the network safer."
      />
      {s.idStatus === "idle" && (
      ...
      {s.idStatus === "verified" && (
        <div className="surface p-5 inline-flex items-center gap-3 text-forest">
          ...
          ID verified. You're cleared to post.
        </div>
      )}
      <p className="mt-4 text-xs text-ink3">
        We use a vendor for this — your document is never stored on our servers.
      </p>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <StepHeader
        ...
        body="Tap each one to expand it. We hold every member to these. Mods enforce them."
      />
...
function StepDone({ s, onFinish }: { s: State; onFinish: () => void }) {
  return (
    <div>
      ...
      <StepHeader
        ...
        body="We've set you up with a private profile. Your next step is to find a Tribe — start with the Lived Experience grid."
      />
      ...
      <div className="mt-6">
        <Button size="lg" onClick={onFinish}>Enter the network</Button>
      </div>
    </div>
  );
}
